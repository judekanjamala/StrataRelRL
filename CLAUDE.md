...
```console
lake build relrl
lake exe relrl verify RelRL/Examples/Assertions.relrl.st  # smoke test: expect 23/23 passed
...
`RelRL/Examples/README.md` is the regression baseline: every example, its
expected goal count, and what it exercises. The two `Map`-equality case studies
there need `--use-array-theory` — see below.
...
| What an example exercises, and its goal count | `RelRL/Examples/README.md` |
...
- **Do comment the non-obvious mechanics**, which a reader cannot recover from
  the code: why a sequence is re-tagged `.newline`. Everything else, delete.
...
- **A `datatype` or multi-function `rec` block in the same file as a `biproc`
  is refused**, by `misaligning_command?` in `Translate/Program.lean`. Not a
  style rule: the body's references would otherwise resolve to the wrong decl,
  or to the literal `0`, and verify a false spec. Widen the guard, never loosen
  it, until the upstream fix lands — `docs/issues.md` has both.
...
- **A goal about `Map` *equality* is `unknown` without `--use-array-theory`.**
  Core's SMT encoder sends `Map` to an uninterpreted sort by default, so
  `select` and `store` work but extensionality and store-over-store do not:
  `m[i := v][i := v] == m[i := v]` does not discharge. The flag encodes `Map` as
  an SMT array instead, and is harmless where it is not needed — reach for it
  the moment a map-valued `Agree` or `==` goes `unknown`.
...
- **Two `Core` namespaces.** `Strata.Core` (the dialect) and `_root_.Core` (the
  verification IR) resolve differently depending on the enclosing namespace,
  which is why `RelRL/Cli/Verify.lean` writes `_root_.Core.VerifyOptions`.
  Wiring RelRL into `Strata-CLI` was abandoned over this; see `docs/design.md`.
...
- **DDM's tokenizer shapes the grammar, so check it before respelling a
  literal.** Which literals can be tokens, why every atom in `Grammar.lean`
  holds exactly one, and which characters cannot start one: `docs/design.md`,
  "## Syntax". Respelling by eye breaks a form elsewhere in the file — and test
  the change, since what looks like an ambiguity often is not one.
...
and out of nothing else. If the two drift, a de Bruijn index resolves against
the wrong binding list and Core aborts with `translateExpr out-of-range bound
variable` — not a type error, and not at the line you changed. Change one,
change the other.
